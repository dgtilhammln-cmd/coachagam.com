...
@section('title', 'Edit Sesi Test')

@section('breadcrumb')
    <span class="breadcrumb-sep">/</span>
    <a href="{{ route('admin.ahp.sessions.index') }}" style="color:#9E9E9E;text-decoration:none;font-size:12px;">Sesi Test</a>
    <span class="breadcrumb-sep">/</span>
    <span class="breadcrumb-current">Edit Sesi</span>
@endsection

@section('content')
<div style="margin-bottom:24px;">
    <h1 class="page-title">Edit Sesi Test</h1>
</div>

<form action="{{ route('admin.ahp.sessions.update', $session) }}" method="POST">
@csrf
@method('PUT')
    ...
    <div class="admin-card-body" style="display:flex;flex-direction:column;gap:18px;">
        ...
        <div class="form-group" style="margin:0;">
            ...
            <select id="label" name="label" class="form-input" required>
                ...
                @foreach($labels as $l)
                <option value="{{ $l }}" {{ old('label', $session->label) == $l ? 'selected' : '' }}>{{ $l }}</option>
                @endforeach
            </select>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
            <div class="form-group" style="margin:0;">
                ...
                <input type="date" id="test_date" name="test_date" class="form-input" value="{{ old('test_date', $session->test_date->format('Y-m-d')) }}" required>
            </div>
            <div class="form-group" style="margin:0;">
                <label class="form-label" for="test_time">Jam Test</label>
                <input type="time" id="test_time" name="test_time" class="form-input" value="{{ old('test_time', $session->test_time) }}">
            </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:2fr 1fr 1fr;gap:16px;">
            <div class="form-group" style="margin:0;">
                ...
                <input type="text" id="location" name="location" class="form-input" value="{{ old('location', $session->location) }}" placeholder="Training Ground RNA">
            </div>
            <div class="form-group" style="margin:0;">
                <label class="form-label" for="temperature">Suhu</label>
                <input type="text" id="temperature" name="temperature" class="form-input" value="{{ old('temperature', $session->temperature) }}" placeholder="26°C">
            </div>
            <div class="form-group" style="margin:0;">
                <label class="form-label" for="period_week">Minggu Ke-</label>
                <input type="number" id="period_week" name="period_week" class="form-input" value="{{ old('period_week', $session->period_week) }}" min="0" placeholder="0">
            </div>
        </div>

        <div class="form-group" style="margin:0;">
            <label class="form-label" for="coach_notes">Catatan Coach</label>
            <textarea id="coach_notes" name="coach_notes" class="form-textarea" rows="3" placeholder="Catatan kondisi cuaca, kondisi lapangan, dll.">{{ old('coach_notes', $session->coach_notes) }}</textarea>
        </div>
    </div>
</div>
